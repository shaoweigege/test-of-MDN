...
export class CountUp {
  ...
  constructor(
    ...
    if (this.el) {
      //这里才是真正去实现动画的函数 this.printValue()
  ...
  // start animation 调用start()才真正的让数字动起来
  start(callback?: (args?: any) => any) {
    if (this.error) {
      return;
    }
    this.callback = callback;
    //this.duration>0==>倒计时还没有结束
    if (this.duration > 0) {
      //此函数用来确定数字的增加还是减少以及动画效果
  ...
  printValue(val: number) {
    //此处的result就是每时每刻应该显示的数值 this.formattingFn()就是将number转成string
    const result = this.formattingFn(val);

    if (this.el.tagName === 'INPUT') {
      const input = this.el as HTMLInputElement;
      input.value = result;
      //???什么元素的tagName是text tspan
