...
// Zepto.js may be freely distributed under the MIT license.
  ...
  //自定义的函数来确定此时的设备类型 接收参数:navigator.userAgent
  function detect(ua) {
    var os = {};
    var browser = {};
    ...
    // It is difficult to detect WeChat in Win Phone precisely, because ua can not be set on win phone. So we do not consider Win Phone.
    if (weChat) {
      browser.weChat = true;
    }
...
// };
  ...
  function containStroke$2(x0, y0, x1, y1, x2, y2, x3, y3, lineWidth, x, y) {
    ...
    // Quick reject
    if (
        (y > y0 + _l && y > y1 + _l && y > y2 + _l && y > y3 + _l) || (y < y0 - _l && y < y1 - _l && y < y2 - _l && y < y3 - _l)
        || (x > x0 + _l && x > x1 + _l && x > x2 + _l && x > x3 + _l) || (x < x0 - _l && x < x1 - _l && x < x2 - _l && x < x3 - _l)
    ) {
      return false;
    }
    ...
    for (var i = 0; i < data.length;) {
      ...
      switch (cmd) {
        ...
        case CMD$1.R:
          ...
          var y1 = y0 + height;
          if (isStroke) {
            if (containStroke$1(x0, y0, x1, y0, lineWidth, x, y) || containStroke$1(x1, y0, x1, y1, lineWidth, x, y)
                || containStroke$1(x1, y1, x0, y1, lineWidth, x, y) || containStroke$1(x0, y1, x0, y0, lineWidth, x, y)) {
              return true;
            }
          } else {
            // FIXME Clockwise ?
...
// ];
  ...
  function processArc(x1, y1, x2, y2, fa, fs, rx, ry, psiDeg, cmd, path) {
    ...
    var f = (fa === fs ? -1 : 1) * mathSqrt((((rx * rx) * (ry * ry)) - ((rx * rx) * (yp * yp))
